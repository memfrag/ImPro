...
void main() {
    ...
    float fuchsiaDistance = distance(lab.gb, vec2(76.25, -1.02));
    float k1 = step(fuchsiaDistance, 20.0);
    
    float neonOrangeDistance = distance(lab.gb, vec2(42.29, 80.10));
    float k2 = step(neonOrangeDistance, 37.0);
    
    float mmmYellowDistance = distance(lab.gb, vec2(8.79, 86.45));
    float k3 = step(mmmYellowDistance, 25.0);
    
    float neonGreenDistance = distance(lab.gb, vec2(-35.21, 79.02));
    float k4 = step(neonGreenDistance, 30.0);
    
    float electricBlueDistance = distance(lab.gb, vec2(-25.68, -18.67));
    float k5 = step(electricBlueDistance, 22.0);
    
    float k = clamp(k1 + k2 + k3 + k4 + k5, 0.0, 1.0);
    
    gl_FragColor = vec4(k * rgb.rgb, 1.0);
}
